...
Status: Working MVP. The conversation engine is complete and tested end-to-end; the WhatsApp connector is the next step (see [Roadmap](#roadmap)).
...
1. Loads which step that customer is on (state is stored in SQLite, so it survives between messages).
2. Validates the answer; if it's invalid, it asks again.
3. Saves the choice and advances to the next step.
4. Returns the bot's next message.
